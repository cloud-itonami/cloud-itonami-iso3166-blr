  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Kita Fabrication ААТ"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Kita Fabrication ААТ" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest restricted-supplier-listed-is-held-and-unoverridable
  (testing "restricted-supplier-list membership -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:restricted-supplier-listed} (-> (store/ledger db) last :basis)))
...
(deftest unp-unverified-is-held-and-unoverridable
  (testing "unverified UNP (Учётный номер плательщика) when required -> HARD hold"
    (let [[db actor] (fresh)
