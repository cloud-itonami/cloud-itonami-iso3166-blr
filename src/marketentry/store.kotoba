(ns marketentry.store
  "SSoT for the Belarus (BLR) market-entry compliance actor, behind a
...
(defn demo-data
  ...
  `:on-restricted-supplier-list?` / `:requires-unp?` / `:unp-verified?`
  are ground truth for the flagship and conditional governor checks."
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Kita Fabrication ААТ" :portal "goszakupki.by"
    ...
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "goszakupki.by"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :on-restricted-supplier-list? false
             :requires-unp? true :unp-verified? true
    ...
    "eng-3" {:id "eng-3" :operator "Minami Systems ТАА" :portal "goszakupki.by"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "BLR" :status :intake}
    "eng-4" {:id "eng-4" :operator "Higashi Export ААТ" :portal "zakupki.butb.by"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :on-restricted-supplier-list? true
             :requires-unp? true :unp-verified? true
             :drafted? false :submitted? false
             :jurisdiction "BLR" :status :intake}
    "eng-5" {:id "eng-5" :operator "Nishi Logistics ТАА" :portal "goszakupki.by"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :on-restricted-supplier-list? false
             :requires-unp? true :unp-verified? false
             :drafted? false :submitted? false
             :jurisdiction "BLR" :status :intake}
    "eng-6" {:id "eng-6" :operator "Chuo Civic Tech ААТ" :portal "zakupki.butb.by"
...
(defn kotobase-store
  ...
  [{:keys [db-api seq-fn]}]
  (->KotobaseStore (persist/store {:db-api db-api :actor "marketentry-blr"})
