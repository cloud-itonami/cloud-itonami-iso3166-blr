(ns marketentry.marketentryllm
  ...
  the Belarus (BLR) public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "unp-verified?=" (:unp-verified? e)
                        " on-restricted-supplier-list?=" (:on-restricted-supplier-list? e)
