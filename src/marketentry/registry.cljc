(ns marketentry.registry
  ...
  Unlike a sibling registry with its own numeric statutory-threshold
  recompute, this namespace carries NO such numeric recompute of its
  own -- BLR's flagship governor check
  (`marketentry.governor/restricted-supplier-listed-violations`) is a
  plain restricted-supplier-list-membership boolean read off the
  engagement record, not a computed formula, so there is nothing here
  for it to call. This is a deliberate, honestly-reported difference in
  check SHAPE, not an oversight -- see `marketentry.governor`'s
  namespace docstring.
